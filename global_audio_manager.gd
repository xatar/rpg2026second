...
func play_music(_audio:AudioStream) -> void:
	...
	await fade_out_and_stop(old_player)
...
func play_and_fade_in(player:AudioStreamPlayer)->void:
	...
	tween.tween_property(player,"volume_db", -30, music_fade_duration )
	pass
	
func fade_out_and_stop(player:AudioStreamPlayer)->void:
	var tween : Tween = create_tween()
	tween.tween_property(player,"volume_db", -80, music_fade_duration )
